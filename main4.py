...
class Ventana2(QDialog):
    ...
    def create_bahia_and_go(self):
        global current_bahia
        # You can customize nom_aux and tipo_aux as needed
        current_bahia = Bahia(nom_aux="Bay1", tipo_aux="Línea")
...
class Ventana2_1(QDialog):
    ...
    def add_tt(self):
        global current_bahia
        tt = TT(nom_aux="TT1")
        current_bahia.agregar_equipo(tt)
        window_manager.show_window(Ventana2_1_1)

    def add_tc(self):
        global current_bahia
        tc = TC(nom_aux="TC1")
        current_bahia.agregar_equipo(tc)
        window_manager.show_window(Ventana2_1_2)

    def add_sl(self):
        global current_bahia
        sl = SeccLinea(nom_aux="SL1")
        current_bahia.agregar_equipo(sl)
        window_manager.show_window(Ventana2_1_3)

    def add_sb(self):
        global current_bahia
        sb = SeccBarra(nom_aux="SB1")
        current_bahia.agregar_equipo(sb)
        window_manager.show_window(Ventana2_1_4)

    def add_in(self):
        global current_bahia
        interruptor = Interruptor(nom_aux="IN1", bahia_aux=current_bahia.nom_aux)
        current_bahia.agregar_equipo(interruptor)
